                    ...
                    <div class="card-body">
                        <div class="header-wrap d-flex justify-content-between">
                            <div class="left-content">
                                <h4 class="header-title">{{__('All Projects')}}  </h4>
                                @can('project-delete')
                                  <x-bulk-action/>
                                @endcan
                            </div>
                            ...
                            <div class="right-content">
                                <a href="{{ route('admin.project.new')}}" class="btn btn-primary">{{__('Add New Project')}}</a>
                            </div>
                             @endcan
                        </div>
                                ...
                                <tbody>
                                    @foreach($projects as $data)
                                        <tr>
                                            ...
                                            <td>{{$data->id}}</td>
                                            <td>{{$data->hasProjectDetail->name}}</td>
                                            <td>{{$data->client->name}}</td>
                                            <td>{{$data->hasRequirement->project_manager->name}}</td>
                                            <td>
                                                @can('project-status')
                                                    @if($data->status==1)
